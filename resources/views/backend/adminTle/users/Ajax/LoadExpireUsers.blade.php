<table class="table table-hover daTatable dataTable deleteFormModal text-center demo-foo-filtering3" data-form="deleteForm" id="users-table">
            <thead>
                <tr style="background-color: #EEE;">
                <th>{{ trans('master.no#') }}</th>
                <th style="padding: 0;"><input type="text" style="margin-bottom: 5px;" name="search" class="form-control" placeholder="{{ trans('master.email') }}" id="search3"></th>
                <th>{{ trans('backend/user.plan') }}</th>
                <th>{{ trans('backend/user.period') }}</th>
                <th>{{trans('backend/user.expired')}}</th>
                <th>{{ trans('backend/user.created') }}</th>
...
<script type="text/javascript">
    $(function(){
        var oTable = $('.demo-foo-filtering3').DataTable({
            "ordering": false
        } );
