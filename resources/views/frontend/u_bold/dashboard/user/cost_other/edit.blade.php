...
@section('content')
    ...
    <form class="form-page-create" role="form" id="idform" method="POST" action="{{ route('cost_other.update' , $data->id ) }}">
        {{ csrf_field() }}
        {{method_field('PATCH')}}
    ...
    {!! Form::open(['method' => 'DELETE','route' => ['cost_other.destroy', $data->id],'style'=>'display:inline' , 'id'=>'deleteForm' , 'class'=>'hidden']) !!}
    {!! Form::close() !!}
    ...
    <div id="invoice_modal" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="full-width-modalLabel" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.cost_other.inc.invoice_modal')
    </div><!-- /.modal -->
           ...
           //$('.new').select2();
            ...
            function getProduct() {
                $('.product_id').select2({
                    ...
                    ajax: {
                        url: "{{route('cost_other.get_product_json') }}",
                        dataType: "json",
                        type: "GET",
...
//        });
            ...
            $('#btnSavePrint , #btndownloadPrint , #btnSendmailPrint').on('click' , function () {
                ...
                $(lada).ladda('start');
                var url = "{{ route('cost_other.update',$data->id) }}";
                $.ajax({
                    type: "PATCH",
                    url: url,
                    data: $("#idform").serialize(),
        ...
        function getProductDetails(num) {
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('cost_other.get_product_data')}}',
                data: { id: productID },
                dataType: 'json',
                success: function (data) {
                    if( data == null) {
