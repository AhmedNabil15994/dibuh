...
<div id="cat-modal" class="modal fade">
            ...
            <div class="modal-body">
                    ...
                    <div class="tabs-vertical-env">
                             ...
                             @foreach($category as $row)        
                                ...
                                @if(count($accountszx) > 0)     
                                    ...
                                    @foreach($accountszx as $item)
                                        <?php                                                                    
                                            $screen_id = \DB::table('accounts_to_screens')->where('account_id', '=' , $item->id)->where('screen_id' , '=' ,'4')->get();
                        ...
                        <ul class="nav tabs-vertical">
                            @foreach($category as $row)   
                            <?php 
                                $accountszx= \DB::table('accounts')->where('account_category_id' , '=', $row->id)->where('is_major','=',0)->orderBy('lineage','ASC')->join('accounts_to_screens','accounts.id','=','accounts_to_screens.account_id')->where('accounts_to_screens.screen_id' , '=' ,4)->get();  
                            ?>
                            @if(count($accountszx) > 0)                       
                                <li class="nav-item">
    ...
    <form class="form-page-create" role="form" id="idform" method="POST" action="{{-- route('cost.store') --}} ">
        {{ csrf_field() }}
                ...
                <div class="col-md-12">
                    <div class="btn-toolbar form-group m-b-0">
                        <div class="pull-right">
                            <button  name="submit" data-style="slide-down"  formaction="{{ route('cost.store_draft') }}" type="submit" class="btn btn-success waves-effect waves-light m-r-5 ladda-button btn-save" led="btn-save">
    ...
    <div id="contact_modal" class="modal fade" taex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.cost.inc.create_contact_modal')
    </div><!-- /.modal -->

    <div id="product_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.cost.inc.create_product_modal')
    </div><!-- /.modal -->
...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        $('.account_id').select2({
            ...
            ajax: {
                url: "{{route('cost.get_account_json') }}" ,
                dataType: "json",
                type: "GET",
...
//            maximumSelectionLength: 1,
        ...
        $("#idform").submit(function(e) {
                if ($(this).attr('state') == 'draft'){
                    ...
                    var url = "{{ route('cost.store_draft') }}"; // the script where you handle the form input.--}}
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(data)
                        {
                            ...
                            if (isNaN(data)){
                                ...
                                $.Notification.autoHideNotify('success', 'top right', 'Saved successfully','Invoice Saved successfully In Costs if you show it in Costs<br>');
                                window.location.href = "{{URL::to('dashboard/cost/list')}}";
                            }
        ...
        $('.BtnSend').on('click',function () {
            ...
            var url = "{{ route('cost.invoice_validation') }}";
            $.ajax({
                type: "POST",
                url: url,
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    if (isNaN(data)){
                        ...
                        setTimeout(function() {
                            window.location.href = "{{route('cost.index')}}";
                        }, 2500);
    ...
    function getAddressById(id) {
        ...
        var url = '{{ route("cost.get_one_contact_address", ":contact_id") }}';
    ...
    function getAccountDetails(num) {
        ...
        $.ajax({
            type: 'GET',
            url: '{{route('cost.get_account_data')}}',
            data: { id: accountID },
            dataType: 'json',
            success: function (data) {
                if( data == null) {
    ...
    function getAccountData( accountID,rowNum) {
               var url = '{{route('cost.get_account_data')}}';
    ...
    function getTaxFields( accountID,rowNum) {
        var url = '{{route('cost.get_tax_fields_view')}}';
        var loaging = '<div id="overlayPagination" class="overlay overlay-x1"><i class="fa fa-spinner fa-pulse fa-fw" ></i></div>';
...
//        $('#total_price').val(Number(totalInvoice).toFixed(2));
    ...
    function getAddress(){
        ...
        var url = '{{ route("cost.get_contact_address", ":contact_id") }}';
        url = url.replace(":contact_id", contactID);
    ...
    function setAddress(id){
        var url = '{{ route("cost.get_contact_address_by_id", ":contact_id") }}';
