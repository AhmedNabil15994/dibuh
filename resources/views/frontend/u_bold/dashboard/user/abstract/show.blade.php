...
@section('page-styles')
    ...
    <style>
        ...
        #customFields thead tr td  , #customFields tbody tr td ,#customFields tbody tr td input ,#customFields tbody tr td option {
            text-align:center !important;
        }

        @media (max-width:768px){
             #customFields tbody tr td span ,#customFields tbody tr td input ,#customFields tbody tr td option{
                     font-size: 11px;
...
@section('content')
                    ...
                    <div class="btn-toolbar form-group m-b-0">
                        <div class="pull-right">
                            <?php 
                                ...
                                $invoices = \DB::table('abstract_invoices')->where('id','=',$inv_id)->first();
                            ?>
                            @if($invoices->rest != 0)
            ...
            <div class="panel panel-default page-panel page-create">
                ...
                <div class="panel-body with-padding">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6">
                        <div class="row">
                            <div class="col-xs-12">
                                <h3 class="pull-left label label-def" data-toggle="paid">دفعات المستخلص</h3>
                            </div>
                        </div>
                        <div id="paid">
                              ...
                              <tbody>
                                <?php 
                                  $id = old('invoice_id') ? old('invoice_id') :isset($data->id)?  $data->id  :'';
                                    $paid = \DB::table('installement_invoices')->where('sales_invoice_id','=',$id)->where('invoice_type','=',1)->get();
                                ?>     
                                @foreach($paid as $row)    
                                    <tr>
                                        <td>{{$row->paid}}</td>
    ...
    <div class="modal fade" id="paid_form" role="dialog">
       <form role="form" id="form" method="POST" action="{{ route('abstract.store_installement') }}">
            {{ csrf_field() }}
            <input type="hidden" name="invoice_id" id="invoice_id" value="{{old('invoice_id') ? old('invoice_id') :isset($data->id)?  $data->id  :''}}">
