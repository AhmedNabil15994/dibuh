...
@section('subnav')
    @include(Config::get('front_theme').'.dashboard.user.abstract.inc.subnav')
@endsection

@section('content')


    <form class="form-page-create" role="form" id="idform" method="POST" action="{{ route('abstract.update' , $data->id ) }}">
        {{ csrf_field() }}
        {{method_field('POST')}}
        ...
        <div class="col-md-offset-1 col-md-10">
                ...
                <div class="col-md-12">
                    <div class="btn-toolbar form-group m-b-0">
                        <div class="pull-right">
                            <button  name="submit" data-style="slide-down"  formaction="{{ route('abstract.update',$data->id) }}" type="submit" class="btn btn-success waves-effect waves-light m-r-5 ladda-button btn-save" led="btn-save">
            ...
            <div class="panel panel-default page-panel page-create">
                ...
                <div class="panel-body with-padding">
                    ...
                    <div class="row">
                        <div class="col-xs-12 ">
                            <h3 class="pull-left label label-def" data-toggle="customer-row">معلومات عن المستخلص والعميل</h3>
                        </div>
                    </div>
                    <div class="row" id="customer-row">
                        ...
                        <div class="col-md-6 col-xs-12">
                            <div class="row">
                                <div class="col-md-6 col-xs-12">
                                    <div class="form-group has-feedback ">
                                        <label for="invoice_number" class="control-label">رقم المستخلص :</label>
                                ...
                                <div class="col-md-6 col-xs-12">
                                    <div class="form-group has-feedback ">
                                        <label for="invoice_date" class="control-label">تاريخ المستخلص :</label>{{-- trans('frontend/sales_invoice.invoice_date')--}}
                                ...
                                <tbody >
                                      ...
                                      @foreach($data_details as $product)
                                          ...
                                          <tr>
                                              ...
                                              <td class="account_calc" id='td_account_{{$index}}'>
                                                  <div class='col-md-12' style='padding: 0;'>
                                                      ...
                                                      <input type="hidden" class="sel_acc_id" name="details[{{$index}}][account]" value="{{ isset($product->account_id) ? $product->account_id : '' }}">
    ...
    {!! Form::open(['method' => 'DELETE','route' => ['abstract.destroy', $data->id],'style'=>'display:inline' , 'id'=>'deleteForm' , 'class'=>'hidden']) !!}
    {!! Form::close() !!}


    <div id="contact_modal" class="modal fade" taex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.abstract.inc.create_contact_modal')
    </div><!-- /.modal -->

    <div id="product_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.abstract.inc.create_product_modal')
    </div><!-- /.modal -->
    ...
    <div id="invoice_modal" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="full-width-modalLabel" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.abstract.inc.invoice_modal')
    </div><!-- /.modal -->
        ...
        jQuery(document).ready(function($) {
            ...
            $('.label-def').click(function () {
             ...
             function getInfo(element){
                var id = $(element).children('option:selected').val();
           ...
           //$('.new').select2();
            ...
            $("#customFields tbody").append("<tr class='tab-row"+rowNum+"'>" 
                        ...
                        +"<td id='td_account_" + rowNum + "'>" 
                            +"<div class='col-md-12' style='padding: 0;'>"
                                +"<select class='form-control account_id select2 new' value1="+rowNum+" onchange='' name='details[" + rowNum + "][account_id]' id='account_id"+rowNum+"'>"
            ...
            function getProduct() {
                $('.product_id').select2({
                    ...
                    ajax: {
                        url: "{{route('abstract.get_product_json') }}",
                        dataType: "json",
                        type: "GET",
...
//            maximumSelectionLength: 1,
                ajax: {
                    url: "{{route('abstract.get_contact_json') }}" ,
                    dataType: "json",
                    type: "GET",
                    data: function (params) {
            ...
            $("#idform").submit(function(e) {
                if ($(this).attr('state') == 'draft'){
                    ...
                    if (status !=1 ){
                        if (old_draft_id ==-1){
                            message =  "<h4>سوف يتم حفظ نسخة من هذة المستخلص متضمنه التعديلات الجديده في مسوده جديده. هل انت متأكد ؟</h4>";
                        }else{
                            message =  "<h4>تذكر انك تحفظ في مسودة جديده التي تم انشائها بنفس بيانات المستخلص وليس على المستخلص نفسها . هل انت متأكد ؟</h4>";
                        }
                        bootbox.confirm({
                            closeButton:false,
                            message:message,
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(data)
                                        {
                                            ...
                                            if (isNaN(data)){
                                                ...
                                                $.Notification.autoHideNotify('success', 'top right', 'Updated successfully','Abstract Updated successfully<br>');
                                            }
                                        },
                                        error: function(data){
                                            setTimeout(function () {
            ...
            $('.BtnSend').on('click',function () {
                ...
                var url = "{{ route('abstract.invoice_validation') }}";
                $.ajax({
                    type: "POST",
                    url: url,
...
//        });
            ...
            $('#btnSavePrint , #btndownloadPrint , #btnSendmailPrint').on('click' , function () {
                ...
                $(lada).ladda('start');
                var url = "{{ route('abstract.update',$data->id) }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: $("#idform").serialize(),
        ...
        function getProductDetails(num) {
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('abstract.get_product_data')}}',
                data: { id: productID },
                dataType: 'json',
                success: function (data) {
                    if( data == null) {
        ...
        function getAddressById(id) {
            ...
            var url = '{{ route("abstract.get_one_contact_address", ":contact_id") }}';
        ...
        function getAccountDetails(num) {
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('abstract.get_account_data')}}',
                data: { id: accountID },
                dataType: 'json',
                success: function (data) {
                    if( data == null) {
...
//                    $("#product_id"+num).parent().parent().children('td').children('.form-group').children('.amount_txt').val(Number(data[0].price).toFixed(2));
        ...
        function getAccountData( accountID,rowNum) {
            var url = '{{route('abstract.get_account_data')}}';
        ...
        function getTaxFields( accountID,rowNum) {
            var url = '{{route('abstract.get_tax_fields_view')}}';
            var loaging = '<div id="overlayPagination" class="overlay overlay-x1"><i class="fa fa-spinner fa-pulse fa-fw" ></i></div>';
...
//        var taxCount=$("#td_taxes_"+num).children().length; // get number of elements of #td_taxes_num
        ...
        function sumTotals(){
            ...
            $('#product-row td .quantity').each(function() {
                ...
                $(object.closest('tr').find('.tax_txt')).each(function() {
                    ...
                    console.log(totalOfTaxes[taxTypeRow[objTaxType[i]]]);
    ...
    //        $('#countOfUnits').html(quantity);
        ...
        function getAddress(){
            ...
            var url = '{{ route("abstract.get_contact_address", ":contact_id") }}';
            url = url.replace(":contact_id", contactID);
        ...
        function setAddress(id){
            var url = '{{ route("abstract.get_contact_address_by_id", ":contact_id") }}';
