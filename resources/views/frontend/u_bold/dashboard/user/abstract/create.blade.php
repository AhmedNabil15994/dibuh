...
<div id="cat-modal" class="modal fade">
            ...
            <div class="modal-body">
                    ...
                    <div class="tabs-vertical-env">
                             ...
                             @foreach($category as $row)        
                                ...
                                @if(count($accountszx) > 0)     
                                    ...
                                    @foreach($accountszx as $item)
                                        <?php                                                                     
                                            $screen_id = \DB::table('accounts_to_screens')->where('account_id', '=' , $item->id)->where('screen_id' , '=' ,'2')->get();
                        ...
                        <ul class="nav tabs-vertical">
                            @foreach($category as $row)   
                            <?php 
                                $accountszx= \DB::table('accounts')->where('category_id' , '=', $row->id)->where('is_major','=',0)->orderBy('lineage','ASC')->join('accounts_to_screens','accounts.id','=','accounts_to_screens.account_id')->where('accounts_to_screens.screen_id' , '=' ,2)->get();  
                            ?>
                            @if(count($accountszx) > 0)                       
                                <li class="nav-item">
    ...
    <form class="form-page-create" role="form" id="idform" method="POST" action="{{-- route('abstract.store') --}} ">
        {{ csrf_field() }}
                ...
                <div class="col-md-12">
                    <div class="btn-toolbar form-group m-b-0">
                        <div class="pull-right">
                            <button  name="submit" data-style="slide-down"  formaction="{{ route('abstract.store_draft') }}" type="submit" class="btn btn-success waves-effect waves-light m-r-5 ladda-button btn-save" led="btn-save">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-xs-12">
                                <div class="form-group has-feedback ">
                                    ...
                                    <input class="form-control" type="text" name="invoice_number" id="invoice_number"  placeholder="رقم المستخلص">
...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        $('.account_id').select2({
            ...
            ajax: {
                url: "{{route('abstract.get_account_json') }}" ,
                dataType: "json",
                type: "GET",
...
//            maximumSelectionLength: 1,
        ...
        $("#idform").submit(function(e) {
                if ($(this).attr('state') == 'draft'){
                    ...
                    var url = "{{ route('abstract.store_draft') }}"; // the script where you handle the form input.--}}
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(data)
                        {
                            ...
                            if (isNaN(data)){
                                ...
                                $.Notification.autoHideNotify('success', 'top right', 'Saved successfully','Invoice Saved successfully In Abstracts if you show it in Abstracts<br>');
                                window.location.href = "{{URL::to('dashboard/abstract/list')}}";
                            }
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    if (isNaN(data)){
                        ...
                        setTimeout(function() {
                            window.location.href = "{{route('abstract.index')}}";
                        }, 2500);
