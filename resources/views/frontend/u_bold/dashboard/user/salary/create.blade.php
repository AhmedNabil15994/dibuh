...
@section('subnav')
    @include(Config::get('front_theme').'.dashboard.user.salary.inc.subnav')
@endsection

@section('content')


    <form class="form-page-create" role="form" id="idform" method="POST" action="{{-- route('salary.store') --}} ">
        {{ csrf_field() }}
                ...
                <div class="col-md-12">
                    <div class="btn-toolbar form-group m-b-0">
                        <div class="pull-right">
                            <button  name="submit" data-style="slide-down"  formaction="{{ route('salary.store_draft') }}" type="submit" class="btn btn-success waves-effect waves-light m-r-5 ladda-button btn-save" led="btn-save">
                                    ...
                                    <tr class="tab-row{{$index}}">
                                        ...
                                        <td id='td_account_{{$index}}'>
                                            <div class='col-md-12' style='padding: 0;'>
                                                ...
                                                <select class="form-control account_id select2" value1="{{$index}}" onchange="getInfo(this)" name='details[{{$index}}][account_id]' id='account_id{{$index}}'>
                                                    ...
                                                    @foreach($accountszx as $item)
                                                        <?php
                                                                ...
                                                                $screen_id = \DB::table('accounts_to_screens')->where('account_id', '=' , $item->id)->where('screen_id' , '=' ,'7')->get();
                                                    ...
                                                    @endforeach
                                                    <?php
                                                        $accounts2= \DB::table('accounts')->orderBy('lineage','ASC')->where('account_category_id' , '=' , '0' )->where('is_major','=',0)->get();  
    ...
    <div id="contact_modal" class="modal fade" taex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.salary.inc.create_contact_modal')
    </div><!-- /.modal -->

    <div id="product_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.salary.inc.create_product_modal')
    </div><!-- /.modal -->
    ...
    jQuery(document).ready(function($) {
           ...
           //$('.new').select2();
            ...
            $("#customFields tbody").append("<tr class='tab-row"+rowNum+"'>" 
                        ...
                        +"<td id='td_account_" + rowNum + "'>" 
                            +"<div class='col-md-12' style='padding: 0;'>"
                                +"<select class='form-control account_id select2 new' value1="+rowNum+" onchange='getInfo(this)' name='details[" + rowNum + "][account_id]' id='account_id"+rowNum+"'>"
                                    +"<option>{{trans('frontend/sales_invoice.account')}}</option>"
                                    @foreach($category as $row)
       ...
       /* function getProduct() {
            $('.product_id').select2({
                ...
                ajax: {
                    url: "{{route('salary.get_product_json') }}",
                    dataType: "json",
                    type: "GET",
...
//            maximumSelectionLength: 1,
            ajax: {
                url: "{{route('salary.get_contact_json') }}" ,
                dataType: "json",
                type: "GET",
                data: function (params) {
        ...
        $("#idform").submit(function(e) {
                if ($(this).attr('state') == 'draft'){
                    ...
                    var url = "{{ route('salary.store_draft') }}"; // the script where you handle the form input.--}}
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(data)
                        {
                            ...
                            if (isNaN(data)){
                                ...
                                $.Notification.autoHideNotify('success', 'top right', 'Saved successfully','Invoice Saved successfully In Salaries if you show it in Salaries<br>');
                                window.location.href = "{{URL::to('dashboard/salary/list')}}";
                            }
        ...
        $('.BtnSend').on('click',function () {
            ...
            var url = "{{ route('salary.invoice_validation') }}";
            $.ajax({
                type: "POST",
                url: url,
    ...
    function getAddressById(id) {
        ...
        var url = '{{ route("salary.get_one_contact_address", ":contact_id") }}';
    ...
    function getAccountDetails(num) {
        ...
        $.ajax({
            type: 'GET',
            url: '{{route('salary.get_account_data')}}',
            data: { id: accountID },
            dataType: 'json',
            success: function (data) {
                if( data == null) {
    ...
    function getAccountData( accountID,rowNum) {
               var url = '{{route('salary.get_account_data')}}';
    ...
    function getTaxFields( accountID,rowNum) {
        var url = '{{route('salary.get_tax_fields_view')}}';
        var loaging = '<div id="overlayPagination" class="overlay overlay-x1"><i class="fa fa-spinner fa-pulse fa-fw" ></i></div>';
...
//        $('#total_price').val(Number(totalInvoice).toFixed(2));
    ...
    function getAddress(){
        ...
        var url = '{{ route("salary.get_contact_address", ":contact_id") }}';
        url = url.replace(":contact_id", contactID);
    ...
    function setAddress(id){
        var url = '{{ route("salary.get_contact_address_by_id", ":contact_id") }}';
