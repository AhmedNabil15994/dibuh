...
<li class="active"> {{trans('frontend/'.$module.'.address_create')}} </li>
@endsection
...
@endif
<h3>{{trans('frontend/'.$module.'.address_create')}}</h3>
<hr>    
{!! Form::open(array('route' => ['contact.address.store', $id],'method'=>'POST')) !!}
...
<div class="form-group">
    ...
    <div class="col-md-8">
        ...
        <a class="btn btn-danger pull-left" href="{{ route('contact.address.index',$id) }}"> {{trans('button.cancel')}}</a>           
    </div>
</div>
{!! Form::close() !!}
