...
<div class="row">
    <div class="col-sm-12">
        ...
        <h4 class="page-title" style="margin-top: -50px;">  تعديل عميل\مورد  </h4>
        <p class=" page-title-alt"> من هنا يمكنك تعديل معلومات {{$data->first_name}} {{$data->last_name}}</p>
    </div>
</div>
 <!-- end row -->
...
<div class="col-md-offset-1 col-md-10">
    <!-- <form action="{{route('contact.store')}}" method="post"> -->
      {!! Form::model($data,['route'=>['contact.update',$data->id],'method'=>'patch']) !!}
        <!-- {{ csrf_field() }} -->
        <div class="panel">
            <div class="panel-body page-create with-padding">
                <div class="row">
                    <div class="col-xs-12 ">
                        <h3 class="pull-left label label-def" data-toggle="customer-row">  تعديل عميل\مورد    </h3>
                    </div>
                </div>
                ...
                @endif
                    ...
                    $customerCheck='';
                    $customerRef='';
                    $found_customer_number='';
                    $supplierCheck='';
                    $supplierRef='';
                    $found_supplier_number='';


                    $contact_to_contact_type=App\Models\ContactsToContactType::where('contact_id',$data->id)->get();
                    foreach($contact_to_contact_type as $ctype){


                        if ($ctype->contact_type_id== 1){
                            $customerCheck = 'checked';
                            $customerRef=$ctype->reference_id;
                            $found_customer_number=$ctype->display_id;

                        }elseif ($ctype->contact_type_id== 2) {
                             $supplierCheck = 'checked';
                              $supplierRef=$ctype->reference_id;
                              $found_supplier_number=$ctype->display_id;
                        }
                    }
                    ?>
                <div class="row">
                    ...
                    <div class="col-sm-3 col-xs-12">
                        <div class="form-group has-feedback ">
                            ...
                            <input class="form-control" name="job" type="text" value="{{$data->job}}">
                    ...
                    <div class="col-sm-3 col-xs-12">
                        <div class="form-group has-feedback ">
                            ...
                            <input class="form-control" name="company" type="text" value="{{$data->company}}">
                    ...
                    <div class="row">
                        <div class="col-sm-3 col-xs-12" id="customer_label" style="{{$customerCheck == 'checked' ? '' : 'display:none;'}}">
                            <div class="form-group  ">
                                ...
                                <input class="form-control" name="customer_code" type="number" placeholder=""  value="{{$found_customer_number==''? $new_customer_number : $found_customer_number }}"readonly>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-3 col-xs-12" id="customer_r_label" style="{{$customerCheck == 'checked' ? '' : 'display:none;'}}">
                            <div class="form-group ">
                                ...
                                <input class="form-control" name="customer_reference_code" type="number" value="{{$customerRef}}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-3 col-xs-12" id="supplier_label" style="{{$supplierCheck == 'checked' ? '' : 'display:none;'}}">
                            <div class="form-group  ">
                                ...
                                <input class="form-control" name="supplier_code" type="number"  value="{{$found_supplier_number==''? $new_supplier_number : $found_supplier_number }}"readonly>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-3 col-xs-12" id="supplier_r_label" style="{{$supplierCheck == 'checked' ? '' : 'display:none;'}}">
                            <div class="form-group ">
                                ...
                                <input class="form-control" name="supplier_reference_code" type="number" placeholder="" value="{{$supplierRef}}"  >
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-12 below_form">
                        ...
                        <div class="tab-content" style="padding: 20px 0">
                            <div class="tab-pane active" id="adresse_tab" >
                              <?php $all_address=App\Models\ContactAddress::where('contact_id',$data->id)->get();  ?>
                                <div id="adresse">
                                    @if(count($all_address))
                                                <?php $index = 0   ?>
                                        @foreach($all_address as $adress)
                                                <?php $index++ ?>
                                                {!! $index > 1 ? '<div class="page-create"><hr></div>' : '' !!}
                                                <div class=" row adresse_block">
                                                <div class="form-group col-xs-11 ">
                                                     <label>اسم الشارع ورقم البيت :</label>
                                                    <input type="text" name="adresse[{{$index}}][address_number]" placeholder="" class="form-control"value="{{$adress->house_no}}">
                                                </div>
                                                ...
                                                <div class="form-group col-xs-3 ">
                                                    ...
                                                    <input type="number" name="adresse[{{$index}}][code_tax]" placeholder="" class="form-control" value="{{$adress->postal_code}}" >
                                                </div>

                                                <div class="form-group col-xs-3">
                                                    <label>اسم المركز :</label>
                                                    <input type="text" name="adresse[{{$index}}][region]" placeholder="" class="form-control" value="{{$adress->city}}" >
                                                </div>
                            ...
                            <div class="tab-pane " id="profile1">
                                 <div class="row">
                                    <div class="contact_details_header col-xs-12">
                                      <?php  $phones=App\Models\Contact_phone::where('contact_id',$data->id)->get(); //dd($phones);?>
                            ...
                            <div class="tab-pane" id="messages1">
                                <div class="bank_information row">
                                    <div class="form-group col-xs-3 ">
                                            <label>IBAN :</label>
                                            <input type="number" name="IBAN"  placeholder="" class="form-control" value="{{$data->iban}}">
                                    </div>

                                    <div class="form-group col-xs-3">
                                        <label>BIC :</label>
                                        <input type="text" name="BIC" placeholder="" class="form-control" value="{{$data->bic}}">
                                    </div>
                                    ...
                                    <div class="form-group col-xs-3">
                                        ...
                                        <input type="text" name="bank_name" placeholder="" class="form-control" value="{{$data->bank_name}}">
                                    </div>

                                    <div class="form-group col-xs-3">
                                        <label>رقم الحساب :</label>
                                        <input type="text" name="bank_number" placeholder="" class="form-control" value="{{$data->bank_number}}">
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane" id="settings1" >
                                ...
                                <div class="row" id="header-row">
                                    <div class="col-md-12 col-xs-12" style="margin-bottom: 15px">
                                        <textarea id="header_text" name="header_text">{{$data->notes}}</textarea>
                                    </div>
                                </div>
                                <!-- End SummerNote -->
                            </div>
                ...
                <div class="row">
                    <div class="form-group text-right m-b-0 ">
                        <button class="btn btn-default waves-effect waves-light col-md-2" type="submit">تعديل</button>
