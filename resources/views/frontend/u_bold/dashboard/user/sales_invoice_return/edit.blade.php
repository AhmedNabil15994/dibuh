...
@section('content')
    ...
    <form class="form-page-create" role="form" id="idform" method="POST" action="{{ route('sales_invoice_return.update' , $data->id ) }}">
        {{ csrf_field() }}
        {{method_field('PATCH')}}
    ...
    {!! Form::open(['method' => 'DELETE','route' => ['sales_invoice_return.destroy', $data->id],'style'=>'display:inline' , 'id'=>'deleteForm' , 'class'=>'hidden']) !!}
    {!! Form::close() !!}
    ...
    <div id="invoice_modal" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="full-width-modalLabel" aria-hidden="true" style="display: none;">
        @include(Config::get('front_theme').'.dashboard.user.sales_invoice_return.inc.invoice_modal')
    </div><!-- /.modal -->
           ...
           //$('.new').select2();
                                    ...
                                    @foreach($category as $row)
                                       <?php 
                                            $accounts= \DB::table('accounts')
                                                            ->where('account_category_id',  $row->id)
                                                            ->select('id', 'account_code', 'name','account_category_id')
                                                            ->get();       
                                        ?>
            ...
            function getProduct() {
                $('.product_id').select2({
                    ...
                    ajax: {
                        url: "{{route('sales_invoice_return.get_product_json') }}",
                        dataType: "json",
                        type: "GET",
...
//        });
            ...
            $('#btnSavePrint , #btndownloadPrint , #btnSendmailPrint').on('click' , function () {
                ...
                $(lada).ladda('start');
                var url = "{{ route('sales_invoice_return.update',$data->id) }}";
                $.ajax({
                    type: "PATCH",
                    url: url,
                    data: $("#idform").serialize(),
        ...
        function getProductDetails(num) {
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('sales_invoice_return.get_product_data')}}',
                data: { id: productID },
                dataType: 'json',
                success: function (data) {
                    if( data == null) {
