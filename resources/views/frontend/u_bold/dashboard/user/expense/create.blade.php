...
@section('subnav')
@include(Config::get('front_theme').'.dashboard.user.expense.inc.subnav')
@endsection
...
<div class="col-md-offset-1 col-md-10" style="margin-bottom: 100px">
    <div class="panel panel-default page-panel page-create">
        ...
        <div class="panel-body with-padding">
            <div class="row">
                <div class="col-xs-12 ">
                    <h3 class="pull-left label label-def" data-toggle="customer-row">    {{ trans('frontend/expense.title')}}      </h3>
                </div>
            </div>
            <div class="row" id="customer-row">
                {!! Form::open(array('route' => 'expense.store','method'=>'POST')) !!}                
                @include(Config::get('front_theme').'.dashboard.user.expense.form')
                <div class="col-md-12 col-xs-12">
                    <div class="form-group">
                        ...
                        <a class="btn w-sm btn-white waves-effect" href="{{ route('expense.index') }}"> {{trans('button.cancel')}}</a>           
                    </div>
                </div>
                {!! Form::close() !!}
...
//            $.get('/api/tax-expense?cat_id=' + account_id, function (data) {
//                $('#tax_id').empty();
//                $('#tax_id').append(data);
//
//
