        ...
        <div class="panel-heading">
            <div class="row">
                ...
                <ul class="panel-nav pull-left">
                    <li><a href="{{route('log.index')}}">{{trans('frontend/dashboard.finance')}}</a></li>
                    <li><a href="{{route('log.income')}}">{{trans('frontend/dashboard.invoices')}}</a></li>
                    <li><a class="active" href="{{route('log.contact')}}">{{trans('frontend/dashboard.contact_manager')}}</a></li>
            ...
            <div class="pbody">
              <table class="table table-hover table-striped " id="demo-foo-filtering" data-page-size="6">
                <thead>
                  ...
                  <tbody>
                  @if(count($add_data) && count($edit_data))

                    @foreach($add_data as $row)

                      <?php 
                            $types = \DB::table('contacts_to_contact_types')->where('contact_id','=',$row['contact_id'])
                      ...
                      <tr class="{{$row['contact_id']}}">
                        <td>{{$row['created_at']}}</td>
                        <td>تم انشاء حساب رقم 
                        @foreach($types as $type) 
                          @if(count($types) == 1 )
                            @if($type->contact_type_id == 1)
                              <?php $result = $type->display_id . " (عميل)"; ?>
                            @elseif($type->contact_type_id == 2)
                              <?php $result = $type->display_id . " (مورد)"; ?>
                            @endif
                          @elseif(count($types) == 2)
                            @if($type->contact_type_id == 1)
                              <?php $result = $type->display_id . " (عميل)"; ?>
                            @elseif($type->contact_type_id == 2)
                              <?php $result2 = $type->display_id . " (مورد)"; ?>
                            @endif
                          @endif
                        @endforeach
                        {{$result}} {{$result2}}</td>
...
@section('page-scripts')
   ...
   <script>
         ...
         jQuery(document).ready(function($) {
            ...
            $('#demo-foo-filtering').DataTable({"order": [[ 0, "desc" ]]});
