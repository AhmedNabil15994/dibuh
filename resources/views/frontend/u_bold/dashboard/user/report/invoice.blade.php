        ...
        <div class="panel-heading">
            <div class="row">
                ...
                <ul class="panel-nav pull-left">
                    <li><a class="active" id="state-draft"  href="{{route('report.invoice')}}">{{trans('frontend/sales_invoice.clients' )}}</a></li>
                    <li><a class=""       id="state-unpaid" href="{{route('report.invoice2')}}">{{trans('frontend/sales_invoice.suppliers')}}</a></li>
                </ul>
            </div>
        </div>
        ...
        <div class="panel-body">
          ...
          <div class="pbody table-responsive">
              <div class="BoxContent">
                <table class="table table-hover daTatable dataTable" id="reports-table" data-page-size="4" style="border: 1px solid #DDD;">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      @if(count($test))
                        @foreach ($test as $row)
                        <?php 
                            $final_cost = \DB::table('sales_invoices')->where('user_id','=',Auth::user()->id)->where('status','=',0)->whereIn('invoice_status_id',array(2,3,5))->where('contact_id','=',$row->contact_id)->sum('total_invoice');

                            $rest = \DB::table('sales_invoices')->where('user_id','=',Auth::user()->id)->where('status','=',0)->whereIn('invoice_status_id',array(2,3,5))->where('contact_id','=',$row->contact_id)->sum('rest');
                        ...
                        ?>
                          ...
                          <?php 
                              ...
                              for ($i=0; $i < count($id) ; $i++) { 
                                $sales = \DB::table('sales_invoices')->where('contact_id' , '=' , $id[$i])->whereIn('invoice_status_id',array(2,3,5))->where('status','=',0)->where('user_id','=',Auth::user()->id)->get();?>
                                @foreach($sales as $one)
                                <tr class="collapse order{{$row->id}}">
                                  <td><a href="{{route('sales_invoice.show',$one->id)}}" style="color: #000;">{{trans('frontend/reports.invoice_number')}} {{$one->invoice_number}}</a></td>
